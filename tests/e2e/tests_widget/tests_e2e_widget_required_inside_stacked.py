...
@tag('new')
class WidgetRequiredInsideStakcedTestCase(TestCase):
    def test_build_new_widget(self):
        self.admin_po.navigations.goto_add_url(models.RequiredStackedInline)
        ...
        for inline in inlines:
            ...
            self.assertTrue(po.is_preview_valid(preview, required=True))

        self.admin_po.change_form.submit_form()

        items = models.RequiredStackedInlineItem.objects.all()
