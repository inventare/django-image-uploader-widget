...
class HTMXRequiredWidgetTests(BaseDragDropTests, HTMXTestCase):
    def setUp(self):
        super().setUp()
        self.widget_po = WidgetPO(self.page)

    def goto_page(self):
        self.admin_po.navigations.goto(reverse("htmx-base"))

    def goto_add_page(self):
        self.goto_page()
        self.load_htmx_widget()

    def test_required_widget_flow(self):
        # empty marker
        ...
        self.assertTrue(thumbs[0].is_valid(required=True))
        ...
        # goto change page
        endpoint = f"{reverse('required')}{self.item.pk}/"
